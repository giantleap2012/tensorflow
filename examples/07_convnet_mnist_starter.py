...
with tf.variable_scope('conv2') as scope:
    ...
    relu2 = tf.nn.relu(conv2 + biases, name=scope.name)

    # output is of dimension BATCH_SIZE x 14 x 14 x 64

with tf.variable_scope('pool2') as scope:
    # similar to pool1, output is of dimension BATCH_SIZE x 7 x 7 x 64
...
with tf.variable_scope('fc') as scope:
    ...
    relu3 = tf.nn.relu(tf.matmul(pool2, w) + b)

    # apply dropout
    fc = tf.nn.dropout(relu3, dropout, name='relu_dropout')
...
with tf.Session() as sess:
    ...
    writer = tf.summary.FileWriter('../../graphs/minst', sess.graph)
    ckpt = tf.train.get_checkpoint_state(os.path.dirname('../../checkpoints/conv_net_mnist/checkpoint'))
    ...
    for index in range(initial_step, n_batches * N_EPOCHS):  # train the model n_epochs times
        ...
        if (index + 1) % SKIP_STEP == 0:
            ...
            total_loss = 0.0
            saver.save(sess, '../../checkpoints/conv_net_mnist/mnist_conv_net', index)
    ...
    for i in range(n_batches):
        ...
        total_correct_predictions += sess.run(accuracy)

    print("Accuracy {0}".format(total_correct_predictions / mnist.test.num_examples))